...
router.delete('/video/:videoId',function(req,res){
    ...
    services.like.unlikeVideo(req.params.videoId,req.session.passport.user.id,(err,numReplaced)=>{
        if(err) return res.status(500).end(err.message);
        res.status(200).end(JSON.stringify(numReplaced));
    })
});
...
router.delete('/comment/:commentId',function(req,res){
    ...
    services.like.unlikeComment(req.params.commentId,req.session.passport.user.id,(err,numReplaced)=>{
        if(err) return res.status(500).end(err.message);
        res.status(200).end(JSON.stringify(numReplaced));
    })
});
